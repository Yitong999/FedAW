...
def noniid(dataset, num_users):
    ...
    :return:
    """
    # 60,000 training imgs -->  3000 imgs/shard X 20 shards
    num_shards, num_imgs = 20, 3000
    idx_shard = [i for i in range(num_shards)]
    dict_users = {i: np.array([]) for i in range(num_users)}
    idxs = np.arange(num_shards*num_imgs)
    labels = dataset.attr[:, 0].numpy()
...
def iid(dataset, num_users):
    ...
    """
    num_items = int(len(dataset)/num_users) #each user has how many items
