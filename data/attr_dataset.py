...
class AttributeDataset(Dataset):
    def __init__(self, root, split, query_attr_idx=None, transform=None, color=None):
        ...
        else: #load each client with specific color
            data_path = os.path.join(root, split, f"mnist_color_{color}.npy")
            self.data = np.load(data_path)
            
            attr_path = os.path.join(root, split, f"mnist_attr_{color}.npy")
            self.attr = torch.LongTensor(np.load(attr_path))
