...
#     valid_data_dir = 'ColoredMNIST-Skewed0.05-Severity4'
    ...
    for step in tqdm(range(main_num_steps)):
        ...
        loss_d_ce = criterion(logit_d, label)
        loss_d_adv_ce = criterion(logit_d_adv, label)
        loss_d_update = loss_d_ce * loss_weight.to(device) + loss_d_adv_ce * loss_weight_adv.to(device) #hacked version
        ...
        if step % main_log_freq == 0:
            ...
            writer.add_scalar("loss/b_gce", loss_b_update.mean(),step)

            writer.add_scalar("loss/d_ce", loss_d_ce.mean(), step)
            writer.add_scalar("loss/d_adv_ce", loss_d_adv_ce.mean(), step)
            writer.add_scalar("loss_d_sum", loss_d_update.mean(), step)
